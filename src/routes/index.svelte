...
<header on:mouseover={()=>read=true} on:mouseout={()=>read=false}>
    <!-- {#if slide == 1}  -->
    <div class="text" class:top-out={slide==1&&!intro} style="--in:300ms; --out:1000ms; --state:{getstate(1)}" class:hide={slide!=1} last on:animationend={switchslide}>
        <h1 style="font-size: 28px; --in:300ms; --out:1000ms; --state:{getstate(1)}" class:top-in={slide==1}>Selamat Datang <span>di website</span></h1>
    ...
    <div class="text" class:hide={slide!=2}>
        <h1 style="font-size: 28px; --in:400ms; --out:1100ms; --state:{getstate(2)}" class:left-in={slide==2 && intro} last on:animationend={switchslide} class:bottom-out={slide==2 && !intro}>GUIDANCE</h1>
    ...
    <div class="text" last class:hide={slide!=3} style="--in:400ms; --out:1200ms; --state:{getstate(3)}" class:right-out={slide==3 && !intro} on:animationend={switchslide}>
        <h1 style="font-size: 28px; --in:400ms; --state:{getstate(3)}" class:top-in={slide==3}>Mudah Diakses</h1>
